...
### Полное аргументированное объяснение
Этот блок кода определяет **API-маршруты (эндпоинты)** сервера. Это "ручки", за которые дергает фронтенд для получения данных или их сохранения.

#### 1. Получение схемы (`GET /api/schema`)
```javascript
...
```
**Построчный разбор:**
1.  `app.get('/api/schema', async (_req, res) => {`: Регистрирует обработчик для GET-запросов по адресу `/api/schema`.
    *   `async`: Функция-обработчик помечена как асинхронная, чтобы внутри можно было использовать `await`.
    *   `_req`: Объект запроса (request). Подчеркивание `_` — соглашение, что переменная не используется.
    *   `res`: Объект ответа (response).
2.  `try {`: Начало блока обработки ошибок.
3.  `const schema = await readJson(SCHEMA_FILE);`: Асинхронно читает файл схемы (`schema.json`) с диска. Ждет завершения чтения.
4.  `res.json(schema);`: Отправляет прочитанный объект схемы клиенту в формате JSON.
5.  `} catch (err) {`: Если чтение файла не удалось (например, файла нет).
6.  `respondError(res, 500, 'Не удалось прочитать файл схемы');`: Отправляет клиенту ошибку 500 (Internal Server Error).
7.  `}`: Конец обработчика.

#### 2. Сохранение показаний (`POST /api/records`)
```javascript
...
```
**Построчный разбор:**
1.  `app.post('/api/records', async (req, res) => {`: Регистрирует обработчик для POST-запросов (отправка данных).
2.  `const username = (req.headers['x-username'] || '').trim();`: Извлекает имя пользователя из специального заголовка `x-username`. Если заголовка нет, берет пустую строку. Удаляет пробелы.
3.  `if (!username) return respondError(res, 400, '...');`: Если имя пользователя не передано, прерывает выполнение и возвращает ошибку 400 (Bad Request).
4.  `const payload = { ...req.body };`: Создает копию данных, присланных клиентом (`req.body`). Оператор `...` (spread) копирует все свойства.
5.  `payload['квартира'] = username;`: Принудительно записывает имя пользователя в поле "квартира" (защита от подделки: пользователь не может отправить данные за соседа).
6.  `payload.meta = { capturedAt: new Date().toISOString(), user: username };`: Добавляет метаданные: точное время получения запроса и пользователя.
7.  `try {`: Начало работы с базой данных.
8.  `const db = await readJson(DB_FILE, []);`: Читает текущую базу данных (массив записей). Если файла нет, возвращает пустой массив `[]`.
9.  `db.push(payload);`: Добавляет новую запись в конец массива.
10. `await writeJson(DB_FILE, db);`: Перезаписывает файл базы данных обновленным массивом.
11. `res.json({ status: 'ok', message: 'Показания сохранены' });`: Отправляет клиенту успешный ответ.
12. `} catch (err) {`: Обработка ошибок записи/чтения.
13. `respondError(res, 500, 'Не удалось сохранить данные');`: Сообщает об ошибке сервера.
14. `}`: Конец обработчика.

#### 3. Получение истории (`GET /api/records`)
```javascript
...
```
**Построчный разбор:**
1.  `app.get('/api/records', async (req, res) => {`: Обработчик GET-запроса истории.
2.  `const username = ...`: Получает имя пользователя из заголовка.
3.  `if (!username) return res.json([]);`: Если пользователь неизвестен, просто возвращает пустой список (безопасность).
4.  `try {`: Начало работы с БД.
5.  `const db = await readJson(DB_FILE, []);`: Читает всю базу данных.
6.  `const ownRecords = db.filter(entry => entry['квартира'] === username);`: **Фильтрация.** Проходит по всем записям и оставляет только те, где поле "квартира" совпадает с текущим пользователем.
7.  `res.json(ownRecords);`: Отправляет отфильтрованный список клиенту.
8.  `} catch (err) {`: Ошибка чтения.
9.  `respondError(res, 500, 'Не удалось прочитать базу');`: Сообщение об ошибке.
10. `}`: Конец обработчика.

### Дополнительная информация: Что такое `async`?
Ключевое слово `async` перед объявлением функции (например, `async (req, res) => { ... }`) делает её **асинхронной**.

1.  **Зачем это здесь:** Операции чтения и записи файлов (`readJson`, `writeJson`) занимают время. В Node.js нельзя блокировать поток выполнения (останавливать весь сервер), пока диск крутится.
2.  **Как работает:**
    *   Функция, помеченная `async`, автоматически возвращает `Promise`.
    *   Внутри такой функции можно использовать ключевое слово `await`.
    *   `await readJson(...)` говорит движку JavaScript: *"Приостанови выполнение **этой конкретной функции**, пока файл не прочитается. А пока можешь обрабатывать другие запросы от других пользователей"*.
    *   Когда файл прочитан, выполнение функции возобновляется со следующей строки.
3.  **Без `async/await`:** Нам пришлось бы писать сложные цепочки колбэков (`.then().then()`), что сделало бы код трудным для чтения ("Callback Hell"). `async/await` позволяет писать асинхронный код так, будто он выполняется линейно.
...
## Вопрос 23
**Какие есть запросы на сервер (по типу GET и POST)? Как они работают? В чем плюсы и минусы каждого типа запроса?**

### Полное аргументированное объяснение
В протоколе HTTP (на котором работает весь веб) существует несколько методов запросов (HTTP Methods). Самые популярные — это **GET** и **POST**, но есть и другие.

#### 1. GET (Получить)
**Как работает:**
Используется для **запроса данных** с сервера. Параметры передаются прямо в адресной строке (URL).
*   *Пример:* `https://example.com/search?q=кот` (мы просим сервер найти "кота").
*   В вашем коде: `fetch('/api/schema')` — это GET-запрос.

**Плюсы:**
*   **Кэширование:** Браузер может сохранить ответ и не скачивать его снова.
*   **История и закладки:** Ссылку с параметрами можно сохранить в закладки или отправить другу (он увидит то же самое).
*   **Простота:** Легко тестировать прямо в адресной строке браузера.

**Минусы:**
*   **Безопасность:** Нельзя передавать пароли, так как они видны в URL и сохраняются в истории браузера.
*   **Ограничение размера:** Длина URL ограничена (обычно около 2000 символов), много данных не передашь.
*   **Только чтение:** Не должен менять состояние на сервере (нельзя использовать для удаления или покупки).

#### 2. POST (Отправить)
**Как работает:**
Используется для **отправки данных** на сервер для обработки (создание новой записи, вход в систему). Данные передаются в **теле запроса** (body), которое скрыто от глаз пользователя.
*   *Пример:* Форма логина, загрузка файла.
*   В вашем коде: `fetch('/api/records', { method: 'POST', ... })`.

**Плюсы:**
*   **Безопасность:** Данные не видны в адресной строке (хотя их все равно можно перехватить, если нет HTTPS).
*   **Объем данных:** Нет жестких ограничений по размеру (можно отправлять файлы, большие тексты).
*   **Типы данных:** Можно отправлять сложный JSON, файлы, XML.

**Минусы:**
*   **Нет кэширования:** По умолчанию браузер не сохраняет ответы POST-запросов.
*   **Нельзя добавить в закладки:** Ссылку на результат отправки формы нельзя просто так сохранить.

#### Другие методы (кратко):
*   **PUT:** Полное обновление существующей записи (заменяет старую на новую).
*   **PATCH:** Частичное обновление (поменять только имя пользователя, не трогая пароль).
...
| :--- | :--- | :--- |
| **Назначение** | Получение данных | Отправка данных / Действие |
| **Данные** | В URL (строка запроса) | В теле запроса (Body) |
| **Видимость данных** | Видны всем в URL | Скрыты в теле |
| **Кэширование** | Да | Нет |
| **Закладки** | Можно сохранить | Нельзя |
| **Ограничение объема** | Да (маленький) | Нет (большой) |
